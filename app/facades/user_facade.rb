class UserFacade
  class << self
    ...
    def update_user(data, user_id)
      data = UserService.patch_data("users/#{user_id.to_i}?name=#{data[:name]}&is_available_for_missions%3F=#{data[:is_available_for_missions]}&rescuer_trailer_capacity=#{data[:rescuer_trailer_capacity]}&phone=#{data[:phone]}")
    end
